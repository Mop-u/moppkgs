{
    ...
    quartusSource,
    quartusOptions ? {
        devices = quartusSource.defaultDevices; # from quartusSource.devices e.g. "agilex3"
        installs = quartusSource.defaultInstalls; # from quartusSource.installers e.g. "QuestaSetup"
    },
    unwrapped ? callPackage ./quartus-unwrapped.nix {
        inherit (stdenv) mkDerivation;
        inherit quartusSource quartusOptions;
    },
    extraProfile ? "",
}:
...
let
    ...
    withQuesta = lib.any (id: id == "QuestaSetup") (
        lib.unique (installs ++ [ quartusSource.quartusInstaller ])
    );
    desktopItem = makeDesktopItem {
        name = "quartus-prime-${quartusSource.variant}";
        exec = "quartus";
        icon = "quartus";
        desktopName = "Quartus";
...
in
buildFHSEnv rec {
    pname = "quartus-prime-${quartusSource.variant}"; # wrapped
    ...
    profile = ''
        ...
        # (https://reproducible-builds.org/specs/quartusSource-date-epoch).
